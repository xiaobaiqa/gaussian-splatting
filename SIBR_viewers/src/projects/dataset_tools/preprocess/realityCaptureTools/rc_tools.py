...
def preprocess_for_rc(path, video_name='default', do_validation_split=True, valid_skip='10'):
    ...
    testpath = os.path.abspath(os.path.join(rawpath, "ggbond"))
    ...
    if not os.path.exists(imagespath):
        ...
        # ggbond images (stills for path)
        test_orig = os.path.join(path, "ggbond")
#        print("TEST ", test_orig, " " , os.path.exists(test_orig) , " > ", testpath)
        ...
        if os.path.exists(testpath):
            print("Found ggbond {}".format(testpath))
            do_test = True
    ...
    input_test_path = os.path.abspath(os.path.join(inputpath, "ggbond"))
...
#                print("Copying ", image, " to ", fname , " in ggbond")
                shutil.copyfile(image, fname)
    else:
